...
const float seedPowerThresholdLower = 10;
const float seedPowerThresholdUpper = 20;
float seedPower;
float BoosTAPower;
float RepumpPower;
float PD4Power;
...
int currState[ShuttersInterlocks] = {0};                  // both variables 'current state' and 'state' are needed for 
int state[buttons] = {1};                                 // the interlock and shutter state reading because there are switches             
int buttonState[buttons] = {1};                           // external to arduino control that effect these states
...
void loop() {
  ...
  seedPower = (seedOutputVal) / 1023.0 * 25;
  BoosTAOutputVal = analogRead(BoosTAOutput_Read);
  BoosTAPower = (BoosTAOutputVal) / 1023.0 * 25;
  RepumpOutputVal = analogRead(RepumpOutput_Read); 
  RepumpPower = (RepumpOutputVal) / 1023.0 * 25;
  PD4OutputVal = analogRead(PD4Output_Read); 
  PD4Power = (PD4OutputVal) / 1023.0 * 25;
  ///////////////////////////////////////////////////////////////////////////////////////
  ...
  currState[LasermetShutter] = digitalRead(LasermetShutterPin_Read);
  currState[RepumpShutter] = digitalRead(RepumpShutterPin_Read);
